...
export function drawBusRoute({ params, mbtaData, resources }: { params: BusRouteParams; mbtaData: any; resources: RenderResources }) {
  ...
  const DESCRIPTION_MAX_WIDTH = pageWidth - (PILL_WIDTH + 2 * LEFT_RIGHT_MARGIN + DESCRIPTION_MARGIN);
  ...
  function drawDescriptionAndReturnHeight(description: string) {
    ...
    const { height } = drawFixedHeightTextLines(
      position,
      lines,
      textSize,
      styleOpts
  ...
  function drawLine(encodedPolylines: any, descriptionHeight: number) {
    ...
    const MAP_HEIGHT =
      HEIGHT -
      (2 * TOP_BOTTOM_MARGIN + pillAndDescriptionHeight + MAP_TOP_MARGIN);
...
function drawFixedSizePill(position: Position, text: string, pillSize: PillSize, textSize: TextSize, styleOpts: StyleOpts) {
  const TEXT = text;
  ...
  const textX =
    PILL_X + (PILL_WIDTH - textPath.bounds.width * textScaleFactor) / 2;
  const textY =
    PILL_Y + (PILL_HEIGHT - textPath.bounds.height * textScaleFactor) / 2;

  textPath.translate(
    new paper.Point(textX - textPath.bounds.x, textY - textPath.bounds.y)
  );
  ...
  const lines = Array.from({ length: n }, (_, i) => {
    ...
    if (
      FILL_STYLE === "INSIDE_CROSSHATCH" ||
      FILL_STYLE === "OUTSIDE_CROSSHATCH"
    ) {
...
function drawFixedHeightTextLines(position: Position, lines: string[], textSize: TextSize2, styleOpts: StyleOpts2) {
  const { x: TEXT_X, y: TEXT_Y } = position;
  const {
    height: LINE_HEIGHT,
    spacingFactor: LINE_SPACING_FACTOR,
    maxWidth: MAX_WIDTH
  } = textSize;
  ...
  const textPaths = rawTextPaths.map((textPath, i) => {
    ...
    textPath.translate(
      new paper.Point(
        targetX - textPath.bounds.x,
        targetY - textPath.bounds.y
      )
    );
  ...
  const totalHeight =
    ...
    (lines.length - 1) * LINE_SPACING_FACTOR * actualLineHeight;
...
function drawFullLines(encodedPolylines: any[], position: Position, mapSize: MapSize, styleOpts: StyleOpts3) {
  ...
  const rawMapPaths = encodedPolylines.map((encoded) => {
    ...
    const pathData =
      "M" +
      geojson.coordinates.map(([pLong, pLat]) => `${pLong},${-pLat}`).join("L");
